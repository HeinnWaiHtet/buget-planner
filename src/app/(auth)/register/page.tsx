...
export default function RegisterPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    // Do not create household here (no service role). Let the user log in
    // and create/join a household from Settings. Redirect to login.
    setLoading(false);
    router.push("/login");
  };
  ...
  return (
    <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-lg ring-1 ring-slate-200">
      <div className="space-y-1">
        <h1 className="text-xl font-semibold text-slate-900">{t("auth.createAccount")}</h1>
        <p className="text-sm text-slate-600">{t("auth.registerDesc")}</p>
      </div>
